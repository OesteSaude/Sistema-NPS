...
// 
// 📸 7. EXPORTAR (CORRIGIDO - SEM FONTE EXTERNA)
// 
...
export async function exportarSlide(event) {
    ...
    try {
        ...
        // Aplica fallback de fonte em todos os elementos
        const aplicarFonte = (node) => {
            if (node instanceof HTMLElement) {
                node.style.fontFamily = 'Arial, Helvetica, sans-serif';
            }
        };
        aplicarFonte(clone);
        clone.querySelectorAll('*').forEach(aplicarFonte);

        // Remove links de fonte remota
...
// Atualiza o bind no escopo global
